...
class Pendulos extends THREE.Object3D {
  constructor(gui,titleGui) {
    ...
    this.boxDown.rotation.z = this.guiControls.rotacionDown;
		...
		this.boxTres = new THREE.Mesh(boxGeom, materialUp)
    this.boxTres.scale.y = this.guiControls.escalaTres;
		this.boxTres.position.y = -this.h*(this.guiControls.escalaDown);
		this.boxTres.rotation.z = this.guiControls.rotacionTres+this.guiControls.rotacionDown;
		
    this.rotation.z = this.guiControls.rotacionUp;

		
		this.boxDown.add(this.boxTres)
		this.boxUp.add(this.boxDown)

    this.add (this.boxUp);
    //this.add (this.boxDown);
		//this.add (this.boxTres)
  }
  ...
  createGUI (gui,titleGui) {
    ...
    this.guiControls = {
      ...
      reset : () => {
        this.guiControls.escalaUp = 1.0;
        this.guiControls.rotacionUp = 0.0;
        this.guiControls.escalaDown = 1.0;
        this.guiControls.rotacionDown = 0.0;
				this.guiControls.escalaTres = 1.0
				this.guiControls.rotacionTres = 0.0
        }
    } 
    ...
    folder.add (this.guiControls, 'rotacionDown', -0.5, 0.5, 0.1).name ('Rotación inferior : ').listen();
		folder.add (this.guiControls, 'escalaTres', 1.0, 2.0, 0.1).name ('Escala tres : ').listen();
    folder.add (this.guiControls, 'rotacionTres', -0.5, 0.5, 0.1).name ('Rotación tres : ').listen();
  ...
  update () {
    ...
    this.boxDown.position.y = -this.h;
    this.boxDown.rotation.z = this.guiControls.rotacionDown;
    this.boxDown.scale.y = this.guiControls.escalaDown;
		
		this.boxTres.position.y = -this.h*(this.guiControls.escalaDown);
    this.boxTres.rotation.z = this.guiControls.rotacionTres;
    this.boxTres.scale.y = this.guiControls.escalaTres;
